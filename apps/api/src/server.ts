...
const shutdown=async()=>{clearInterval(scheduler);server.close(async()=>{await Promise.all(workers.map(worker=>worker.close()));await closeNotificationHub();await closeQueues();jobConnection.disconnect();await Promise.all([prisma.$disconnect(),closeRedis()]);process.exit(0);});};
