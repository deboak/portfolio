...
const schema = z.object({
  ...
  DATABASE_URL: z
    .string()
    .min(1)
    .default('postgresql://portfolio:portfolio@localhost:5432/portfolio?schema=public'),
  ...
  OUTGOING_WEBHOOK_SECRET: z.preprocess(
    (value) => (value === '' ? undefined : value),
    z.string().min(32).optional(),
  ),
  INCOMING_WEBHOOK_SECRET: z.preprocess(
    (value) => (value === '' ? undefined : value),
    z.string().min(32).optional(),
  ),
...
if (env.NODE_ENV === 'production' && !env.INCOMING_WEBHOOK_SECRET)
  throw new Error('INCOMING_WEBHOOK_SECRET is required in production');
