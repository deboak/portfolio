...
async function refresh() {
  let token = csrf();
  if (!token) {
    const setup = await raw('/auth/csrf');
    if (!setup.ok) return false;
    token = (await setup.json()).data.csrfToken;
...
export async function streamAdminNotifications(
  signal: AbortSignal,
  onNotification: (event: LiveNotification) => void,
  onConnected?: () => void,
) {
  if (!accessToken && !(await refresh())) throw new Error('Authentication required');
  let response = await raw('/admin/notifications/stream', {
    headers: { authorization: `Bearer ${accessToken}` },
    signal,
  });
  if (response.status === 401 && (await refresh()))
    response = await raw('/admin/notifications/stream', {
      headers: { authorization: `Bearer ${accessToken}` },
      signal,
    });
  if (!response.ok || !response.body)
    throw new Error(`Live notifications unavailable (${response.status})`);
  ...
  while (true) {
    ...
    while (boundary >= 0) {
      ...
      if (event === 'notification' && data.length) {
        try {
          onNotification(JSON.parse(data.join('\n')) as LiveNotification);
        } catch {}
      }
    }
  }
