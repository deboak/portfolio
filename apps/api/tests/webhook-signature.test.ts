...
describe('webhook signatures',()=>{
  it('accepts an authentic payload within the allowed time window',()=>{
    ...
    expect(verifyWebhookSignature({secret,timestamp,rawBody,signature,now:1_000,toleranceSeconds:300})).toBe(true);
  });
  it('rejects payload tampering',()=>{
    const signature=signWebhook(secret,timestamp,rawBody);
    expect(verifyWebhookSignature({secret,timestamp,rawBody:Buffer.from('{}'),signature,now:1_000,toleranceSeconds:300})).toBe(false);
  });
  it('rejects expired deliveries to limit replay attacks',()=>{
    const signature=signWebhook(secret,timestamp,rawBody);
    expect(verifyWebhookSignature({secret,timestamp,rawBody,signature,now:1_000_000,toleranceSeconds:300})).toBe(false);
  });
});
